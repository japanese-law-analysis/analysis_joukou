...
pub fn get_law_abbreviation(article: &Article, text: &str) -> Vec<Abbreviation> {
  let mut lst = Vec::new();
  let re = Regex::new(
...
pub async fn get_law_all_abbreviation(law_xml: &[u8]) -> Result<Vec<Abbreviation>> {
  ...
  while let Some(law_text) = text_stream.next().await {
    ...
    match contents {
      LawContents::Text(text) => {
        let mut l = get_law_abbreviation(&law_text.article_info, &text);
        lst.append(&mut l);
      }
      LawContents::Table(_) => {
